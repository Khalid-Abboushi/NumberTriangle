...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        for (int r = 0; r < levels.size() - 1; r++) {
            List<NumberTriangle> current = levels.get(r);
            List<NumberTriangle> next = levels.get(r + 1);
            for (int j = 0; j < current.size(); j++) {
                NumberTriangle node = current.get(j);
                node.setLeft(next.get(j));
                node.setRight(next.get(j + 1));
            }
        }
